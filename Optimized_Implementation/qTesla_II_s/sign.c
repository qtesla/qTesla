...
void make_Hint(unsigned char *hint, poly v, poly Tc)
{ // Hint generation for public key splitting
  poly vt;
  int32_t mask, cL;
  ...
  for (int i=0; i<PARAM_N; i++) { 
    ...
    // If vt[i] > PARAM_Q/2 then vt[i] -= PARAM_Q
    mask = (PARAM_Q/2 - vt[i]) >> (RADIX32-1);                    
    vt[i] = ((vt[i]-PARAM_Q) & mask) | (vt[i] & ~mask);
    // If vt[i] < -PARAM_Q/2 then vt[i] += PARAM_Q
    mask = (PARAM_Q/2 + vt[i]) >> (RADIX32-1);                    
    vt[i] = ((vt[i]+PARAM_Q) & mask) | (vt[i] & ~mask);
    ...
    mask = ((1<<(PARAM_D-1)) - cL) >> (RADIX32-1);                    
    cL = ((cL-(1<<PARAM_D)) & mask) | (cL & ~mask);
    vt[i] = v[i] - ((vt[i] - cL) >> PARAM_D);
  } 
  for (int i=0, j=0; j<PARAM_N; i+=3, j+=8) { 
      hint[i]   = (vt[j]          & 7) | ((vt[j+1] & 7) << 3) | (vt[j+2]       << 6); 
      hint[i+1] = ((vt[j+2] >> 2) & 1) | ((vt[j+3] & 7) << 1) | ((vt[j+4] & 7) << 4) | (vt[j+5] << 7); 
      hint[i+2] = ((vt[j+5] >> 1) & 3) | ((vt[j+6] & 7) << 2) | (vt[j+7]       << 5); 
  }
}


void hash_H_hint(unsigned char *c_bin, poly v, unsigned char *hint, const unsigned char *hm)
{ // Hash-based function H to generate c' using hint
  unsigned char t[PARAM_N+HM_BYTES];
  int32_t mask, cL;
  signed char h[PARAM_N];
   
  for (int i=0, j=0; i<PARAM_N; i+=8, j+=3) { 
      h[i]   =  (signed char)(hint[j]   << 5) >> 5; 
      h[i+1] =  (signed char)(hint[j]   << 2) >> 5; 
      h[i+2] = ((signed char)(hint[j+1] << 7) >> 5) | (hint[j]   >> 6); 
      h[i+3] =  (signed char)(hint[j+1] << 4) >> 5;  
      h[i+4] =  (signed char)(hint[j+1] << 1) >> 5; 
      h[i+5] = ((signed char)(hint[j+2] << 6) >> 5) | (hint[j+1] >> 7);  
      h[i+6] =  (signed char)(hint[j+2] << 3) >> 5;   
      h[i+7] =  (signed char)(hint[j+2]     ) >> 5; 
  }
  for (int i=0; i<PARAM_N; i++) { 
    // If v[i] > PARAM_Q/2 then v[i] -= PARAM_Q
    mask = (PARAM_Q/2 - v[i]) >> (RADIX32-1);                    
    ...
    mask = ((1<<(PARAM_D-1)) - cL) >> (RADIX32-1);                    
    cL = ((cL-(1<<PARAM_D)) & mask) | (cL & ~mask);  
    //t[i] = (unsigned char)((v[i] - cL) >> PARAM_D) + ((h[i>>3] >> (i & 7)) & 1);        
    t[i] = (v[i] - cL) >> PARAM_D;
    t[i] += h[i];
  } 
  memcpy(&t[PARAM_N], hm, HM_BYTES);
...
#if defined(SPLIT_KEY)
  encode_sk(sk, s, e, &randomness_extended[2*CRYPTO_SEEDBYTES], t);
#else
...
#if defined(SPLIT_KEY)
  poly Tc, t0;
  unsigned char hint[3*PARAM_N/8];
#endif
#ifdef DEBUG
  ctr_sign=0;
  rejwctr=0;
...
#if defined(SPLIT_KEY)
    //sparse_mul32(Tc, t0, pos_list, sign_list); 
    poly_mul(Tc, c_ntt, t0);
    make_Hint(hint, v, Tc);
    encode_sig(sm, c, z, hint);
#else    
    encode_sig(sm, c, z);
#endif
...
int crypto_sign_open(unsigned char *m, unsigned long long *mlen, const unsigned char *sm, unsigned long long smlen, const unsigned char *pk)
{
  ...
  poly w, z, a, Tc;
#if defined(SPLIT_KEY)
  unsigned char hint[3*PARAM_N/8] = {0};
#endif

  if (smlen < CRYPTO_BYTES) return -1;
  
#if defined(SPLIT_KEY)
  decode_sig(c, z, hint, sm);  
#else
  decode_sig(c, z, sm);  
#endif
...
#if defined(SPLIT_KEY)
  for (int i=0; i < PARAM_N; i++) pk_t[i] <<= 16;  
#endif       
